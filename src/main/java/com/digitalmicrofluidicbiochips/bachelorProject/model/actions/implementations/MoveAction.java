...
public class MoveAction extends ActionBase {
    ...
    public ActionTickResult executeTick(ProgramConfiguration programConfiguration) {
        ...
        // If the droplet is blocked, attempt to resolve the deadlock.
        if(move == DropletMove.BLOCKED && isAttemptToResolveDeadlock()) {
            move = getValidRandomDropletMove(programConfiguration, availableElectrodeGrid);
        }
     ...
     */
    private DropletMove getValidRandomDropletMove(ProgramConfiguration programConfiguration, ElectrodeGrid availableElectrodeGrid) {
        IPathFinder pathFinder = programConfiguration.getPathFinder();
